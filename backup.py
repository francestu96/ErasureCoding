...
K = 7 # number of blocks needed to recover data
...
for k in range(K, N + 1):
    plot_speed = []
    plot_time = []
    for upload_speed, download_speed in zip(range(5*KB, UPLOAD_SPEED + 5*KB, 5*KB), range(20*KB, DOWNLOAD_SPEED + 20*KB, 20*KB)):
        UPLOAD_DURATION = BLOCK_SIZE / upload_speed
        DOWNLOAD_DURATION = BLOCK_SIZE / download_speed
    ...
    plt.plot(plot_time, plot_speed, label=f"Redundancy: {N/k:.2f}")
